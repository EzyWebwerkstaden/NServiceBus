namespace NServiceBus.Core.Tests.Reliability.Outbox
{
    ...
    public class TransportReceiveToPhysicalMessageProcessingConnectorTests
    {
        ...
        {
            var context = new TransportReceiveContext(new IncomingMessage("id", new Dictionary<string, string>(), new MemoryStream()), "InputQueue", null, new CancellationTokenSource(), new RootContext(null, new FakePipelineCache(pipeline)));
            return context;
        }
